...
- **Static API tokens** (`api-tokens` module, `/api/tokens` — list/create/revoke, JwtAuthGuard, owner-scoped): a user can mint long-lived personal access tokens to drive the admin API without login/password. A token **is a normal login JWT** (same secret, same `sub/email/role` payload — so it carries the owner's role and flows through the unchanged `JwtAuthGuard`/`AdminGuard`), differing only in a ~999-year `expiresIn` and a `jti` claim equal to its `api_tokens` row id. The plaintext token is returned **once** at creation (`CreatedApiTokenDto`) and never stored — only metadata (name, `lastUsedAt`, `expiresAt`) lives in the DB. [JwtStrategy](src/auth/jwt.strategy.ts) calls `ApiTokensService.assertActive(jti)` on every request when a `jti` is present, so **deleting the row instantly revokes the token** (401). `ApiTokensModule` is imported by `AuthModule` (not vice-versa, to stay acyclic) and signs its own JWTs via a locally-registered `JwtModule`.

### Data layer: Prisma 7 (critical)

- Schema: [prisma/schema.prisma](prisma/schema.prisma). Models `User`, `Bot`, `DeliveryLog`, `ApiToken` (+ `UserRole` enum). `@@map`/`@map` keep the original snake_case table/column names.
...
- **Route groups**: `(marketing)` = public landing `/` + docs `/guide`; `(app)` = authed shell — `/dashboard` (admin), `/bots`, `/tokens` (static API tokens, all users), `/users` (admin). The `(app)` layout fetches `/auth/me` and renders **role-based nav** (Dashboard/Users only for admins). `/login`, `/register`, `/verify` use the minimal root layout. Only the root layout renders `<html>/<body>`.
