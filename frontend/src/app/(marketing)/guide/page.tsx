  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-14">
      <header className="mb-10">
        ...
        <h1 className="mt-2 text-4xl font-bold tracking-tight text-white">Интеграция за пять минут</h1>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-lg bg-tg-500 px-4 py-2 text-sm font-semibold text-[#06243a] transition hover:bg-tg-400"
          >
            Открыть Swagger API ↗
          </a>
          <Link href="/login" className="rounded-lg border border-white/15 px-4 py-2 text-sm text-slate-200 hover:bg-white/5">
            Войти в админку
          </Link>
        </div>
      </header>
      ...
      <div className="grid gap-10 lg:grid-cols-[180px_1fr]">
        ...
        <aside className="hidden lg:block">
          <nav className="sticky top-24 space-y-1.5 border-l border-white/10 pl-4 text-sm">
            {toc.map(([id, n, label]) => (
              <a key={id} href={`#${id}`} className="flex items-baseline gap-2 text-slate-400 transition hover:text-white">
        ...
        <div>
          <Section id="intro" n="00" title="О прокси">
            <p>
              Прокси решает две задачи. <strong className="text-white">Во-первых</strong>, принимает вебхуки от
              Telegram на зарубежном сервере и пересылает их на ваш «реальный» бэкенд.{' '}
              <strong className="text-white">Во-вторых</strong>, служит drop-in заменой{' '}
            ...
            <p>
              ...
              Замените на свой домен после деплоя.
            </p>
          </Section>
          ...
          <Section id="outbound" n="01" title="Исходящий Bot API (drop-in замена хоста)">
            <p>
              ...
              на адрес прокси. Проксируются <strong className="text-white">все</strong> методы и любой content-type,
          ...
          <Section id="webhooks" n="03" title="Входящие вебхуки">
            <p>
              ...
              <strong className="text-white">как есть</strong> пересылается на ваш{' '}
            ...
            <Callout>
              Верните <strong className="text-white">2xx</strong> как можно быстрее. Если бэкенд ответил не-2xx или
          ...
          <Section id="api" n="05" title="Справочник API">
            ...
            <div>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-lg border border-tg-500/40 bg-tg-500/10 px-4 py-2.5 text-sm font-medium text-tg-200 transition hover:bg-tg-500/20"
              >
                Открыть Swagger UI ↗
              </a>
            </div>
